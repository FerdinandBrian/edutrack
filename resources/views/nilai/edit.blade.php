...
<div class="bg-white rounded-xl shadow p-6 max-w-2xl">
    ...
    <form action="/nilai/{{ $nilai->id }}" method="POST" class="mt-4 space-y-4">
        @csrf
        @method('PUT')
        <div>
            ...
            <select name="nrp" required class="w-full border rounded px-3 py-2">
                ...
                @foreach($mahasiswas as $m)
                    <option value="{{ $m->nrp }}" {{ $m->nrp == $nilai->nrp ? 'selected' : '' }}>{{ $m->nrp }} — {{ $m->nama }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <input name="kode_mk" value="{{ $nilai->kode_mk }}" required class="w-full border rounded px-3 py-2">
        </div>

        <div>
            <label class="block text-sm text-slate-700 mb-1">Nilai</label>
            <input name="nilai" value="{{ $nilai->nilai }}" required type="number" step="0.01" class="w-full border rounded px-3 py-2">
        </div>
