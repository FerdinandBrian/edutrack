...
<div class="bg-white rounded-xl shadow p-6">
    <div class="flex items-center justify-between">
        ...
        @if(auth()->user()->id_role == 2)
            <a href="/presensi/create" class="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm">Tambah Presensi</a>
        @endif
    </div>
        ...
        <table class="w-full table-auto">
            ...
            <tbody>
                ...
                <tr class="border-b">
                    ...
                    <td class="text-right">
                        <a href="/presensi/{{ $row->id }}" class="text-sm text-blue-600 mr-2">Lihat</a>
                        @if(auth()->user()->id_role == 2)
                            <a href="/presensi/{{ $row->id }}/edit" class="text-sm text-yellow-600 mr-2">Edit</a>
                            <form action="/presensi/{{ $row->id }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus presensi ini?')">
                                @csrf
                                @method('DELETE')
